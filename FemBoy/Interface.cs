...
public static class Interface {
    ...
    public static string PrintRegisters() {
        if (gb.gameboy == null || gb.gameboy.CPU == null) return "";
        string output =
            $"[Registers]\n" +
            ...
            */
            $"[AF] {gb.gameboy.CPU.Registers.AF:X4} [BC] {gb.gameboy.CPU.Registers.BC:X4}\n" +
            $"[DE] {gb.gameboy.CPU.Registers.DE:X4} [HL] {gb.gameboy.CPU.Registers.HL:X4}\n" +
            $"[Z] {(gb.gameboy.CPU.Registers.GetFlag(CPUFlagMask.Zero) ? "1":"0")} [N] {(gb.gameboy.CPU.Registers.GetFlag(CPUFlagMask.Negative) ? "1":"0")} [H] {(gb.gameboy.CPU.Registers.GetFlag(CPUFlagMask.HalfCarry) ? "1":"0")} [C] {(gb.gameboy.CPU.Registers.GetFlag(CPUFlagMask.Carry) ? "1":"0")}\n" +
            $"[Frame] {gb.gameboy.CPU.ops}\n" +
    ...
    public static string PrintPPUInfo() {
        if (gb.gameboy == null) return "";
        string output = "[PPU]\n";
        output += $"[Mode] {gb.gameboy.PPU.Mode}\n";
    ...
    public static void Load() {
        ...
        Draw2DOnTop += (DrawShapesToSurface draw_shapes) => {
            ...
            if (!MouseHidden) {
                draw_shapes.draw_shape_single_color(UIGraphics.cursor, cursor_shadow_offset, UIColors.Shadow,
                    Color.Transparent, 0, sdf_pattern.DITHER, 1);
